...
ThreadPool::ThreadPool(int min, int max) : capacity(100) {
	...
	for (int i = 0; i < min; i++) {
		this->workThreads.push_back(thread(&ThreadPool::worker, this));
	}
...
void ThreadPool::worker() {
	while (true) {
		...
		unique_lock<mutex> busyLock(busyMutex_);
		busyNum++;
		busyLock.unlock();
	...
	while (!shutdown) {
		...
		if (queueSize > liveNum && liveNum < maxNum) {
			...
			for (int i = 0; i < maxNum && counter < 2 && liveNum < maxNum; i++) {
				if (workThreads[i].joinable() == false) {
					workThreads[i] = thread(&ThreadPool::worker, this);
					counter++;
					liveNum++;
				}
			}
		...
		if (busyNum * 2 < liveNum && liveNum > minNum) {
			...
			if (liveNum > minNum) {
				...
				for (int i = 0; i < workThreads.size() && counter < exitNum; i++) {
					if (workThreads[i].joinable() == true) {
						workThreads[i].join();
						counter++;
						liveNum--;
					}
...
int ThreadPool::getBusyNum() { 
	unique_lock<mutex> lock(busyMutex_); int busyNum = this->busyNum; lock.unlock(); return busyNum;
}

int ThreadPool::getLiveNum() { 
	unique_lock<mutex> lock(poolMutex_); int liveNum = this->liveNum; lock.unlock(); return liveNum; 
}

ThreadPool::~ThreadPool() {
	shutdown = true; cond_Var.notify_all();
